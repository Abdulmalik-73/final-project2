<?php
/**
 * Ensure checkins table exists in database
 * This file is included before any check-in operations
 */

function ensure_checkins_table_exists($conn) {
    ...
    if (!$table_check || $table_check->num_rows === 0) {
        ...
        if ($conn->query($create_table_sql)) {
            error_log("✅ Checkins table created successfully");
            return true;
        } else {
            error_log("❌ Failed to create checkins table: " . $conn->error);
            return false;
        }
    }
